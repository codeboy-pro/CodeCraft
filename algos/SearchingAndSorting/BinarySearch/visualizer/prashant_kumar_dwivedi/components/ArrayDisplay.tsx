...
export default function ArrayDisplay({ array, current }: ArrayDisplayProps) {
  return (
    <div className="mb-6 min-h-[120px] p-4 bg-white border-2 border-gray-200 rounded-lg flex flex-wrap justify-center items-center gap-2">
      {array.length > 0 ? (
        array.map((num, index) => {
          let className = "border-2 px-3 py-2 text-center min-w-[60px] rounded font-mono transition-all";
          if (current) {
        ...
        <div className="text-gray-500 text-center">
          ...
          <p className="text-sm">Enter an array above and click "Set Array" or "Generate Random Array"</p>
        </div>
      )}
    </div>
  );
